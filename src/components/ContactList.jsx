...
const ContactCategory = ({ title, contacts, count }) => {
  ...
  return (
    <div className="mt-2">
      <div
        className="flex items-center cursor-pointer ml-1 hovercontact border border-transparent"
        onClick={toggleAccordion}
      >
        <h2>
          {isOpen ? (
            <img src={closedTabArrow} alt="close tab" />
          ) : (
...
const Contacts = ({ contact }) => {
  ...
  return (
    <div
      className="flex gap-1 px-6 items-center hovercontact border border-transparent"
